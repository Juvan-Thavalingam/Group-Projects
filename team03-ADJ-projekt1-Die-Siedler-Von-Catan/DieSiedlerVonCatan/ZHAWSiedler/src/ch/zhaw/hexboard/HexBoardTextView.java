...
public class HexBoardTextView<F, C, E, A> {
  ...
  private static final int MAX_X_AXIS = 14;
  ...
  private String printMiddlePartOfField(int y) {
    ...
    //SPOT
    if (y == board.getMaxCoordinateY() + 1) {
      StringToInsert = "Y /";
    } else {
      StringToInsert = TWO_SPACES;
    }
    return upper + System.lineSeparator() + StringToInsert + THREE_SPACES + lower;
  }
  ...
  private String convertDigitsToCorrectFormat(int number) {
    String NumberAsString;
    if (Integer.toString(number).length() == 2) {
      NumberAsString = number + THREE_SPACES;
    } else {
      NumberAsString = number + FOUR_SPACES;
    }

    return NumberAsString;
  }

  private StringBuilder createXAxisNumbers(StringBuilder sbtemp) {
    sbtemp.append("/ X" + THREE_SPACES);
    for (int cycle = 0; cycle < MAX_X_AXIS + 1; cycle++) {
      sbtemp.append(cycle);
      if (Integer.toString(cycle).length() == 2) {
        sbtemp.append(SEVEN_SPACES);
      } else {
        sbtemp.append(SEVEN_SPACES + ONE_SPACE);
      }
    }
  ...
  public String toString() {
    ...
    return createXAxisNumbers(sb).toString();
  }
